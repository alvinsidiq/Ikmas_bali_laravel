    ...
    <form method="POST" action="{{ route('anggota.laporan.update',$laporan) }}" class="space-y-4">
      ...
      <div>
        ...
        <select id="kategori" name="kategori" class="mt-1 block w-full border-gray-300 rounded">
          ...
          <option value="Laporan Kegiatan" @selected($laporan->kategori==='Laporan Kegiatan')>Laporan Kegiatan</option>
          <option value="Laporan Pengumuman" @selected($laporan->kategori==='Laporan Pengumuman')>Laporan Pengumuman</option>
        </select>
      </div>
      <div>
